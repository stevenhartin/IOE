...
namespace Renderer
{
	...
	bool IOETexture_Platform::GetDescription1D(D3D11_TEXTURE1D_DESC &rDesc, std::uint32_t uNumMipMaps,
											   bool bStaging /*=false*/) const
	{
			...
			GetTexture().GetTextureType() == ETextureType::Texture1DArray)
		{
			...
			rDesc.Usage			 = GetCPUAccess(GetTexture().GetBufferAccess(), bStaging);
			rDesc.BindFlags		 = GetBindFlag(GetTexture().GetBufferDescription());
			rDesc.CPUAccessFlags = GetTextureAccessType(GetTexture().GetBufferAccess(), bStaging);
			rDesc.MiscFlags		 = 0;
	...
	bool IOETexture_Platform::GetDescription2D(D3D11_TEXTURE2D_DESC &rDesc, std::uint32_t uNumMipMaps,
											   bool bStaging /*=false*/) const
	{
	...
	bool IOETexture_Platform::GetDescription3D(D3D11_TEXTURE3D_DESC &rDesc, std::uint32_t uNumMipMaps,
											   bool bStaging /*=false*/) const
	{
	...
	bool IOETexture_Platform::GetDescriptionCube(D3D11_TEXTURE2D_DESC &rDesc, std::uint32_t uNumMipMaps,
												 bool bStaging /*=false*/) const
	{
		if (GetTexture().GetTextureType() == ETextureType::TextureCube)
		{
			...
			rDesc.BindFlags			 = GetBindFlag(GetTexture().GetBufferDescription());
			rDesc.CPUAccessFlags	 = GetTextureAccessType(GetTexture().GetBufferAccess(), bStaging);
			rDesc.ArraySize			 = 6 * Maths::Max(static_cast<UINT>(GetTexture().GetArrayLength()), 1U);
			rDesc.SampleDesc.Quality = 0;
			rDesc.SampleDesc.Count   = 1;
	...
	bool IOETexture_Platform::GetDescriptionBuffer(D3D11_BUFFER_DESC &rDesc, bool bStaging /*=false*/) const
	{
		if (GetTexture().GetTextureType() == ETextureType::GenericBuffer)
		{
			...
			if (GetTexture().CanAllowRawValues())
			{
				...
				rDesc.ByteWidth =
					static_cast<UINT>(GetTexture().GetWidth()) * static_cast<UINT>(GetTexture().GetArrayLength());
				if (!bStaging)
					rDesc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_ALLOW_RAW_VIEWS;
			}
			else
			{
				rDesc.StructureByteStride = static_cast<UINT>(GetTexture().GetWidth());
				rDesc.ByteWidth = static_cast<UINT>(GetTexture().GetArrayLength()) * rDesc.StructureByteStride;
				if (!bStaging)
	...
	bool IOETexture_Platform::MapBuffer(EBufferMapType eMapType, void *&rpData, std::uint32_t uSubresourceID /*=0*/)
	{
		ID3D11DeviceContext *pDeviceContext(
			IOE::Renderer::IOERPI::GetSingletonPtr()->GetPlatform().GetDeviceContext());

		ID3D11Resource *pTexture(m_pTexture.get());

		if (m_pStagingTexture != nullptr && GetTexture().GetBufferAccess() == ECPUAccess::GPUWriteBack)
		{
			pDeviceContext->CopyResource(m_pStagingTexture.get(), m_pTexture.get());

			pTexture = m_pStagingTexture.get();
		}

		D3D11_MAPPED_SUBRESOURCE tMappedSubresource;
		if (SUCCEEDED(pDeviceContext->Map(pTexture, uSubresourceID, GetMapType(eMapType), uSubresourceID,
										  &tMappedSubresource)))
		{
	...
	bool IOETexture_Platform::Setup(IOETextureSource *pSource /*=nullptr*/)
	{
		...
		IOE_ASSERT(IOE::Renderer::IOERPI::GetSingletonPtr()->GetPlatform().GetDevice() == nullptr,
	...
	bool IOETexture_Platform::CreateTexture(IOETextureSource *pSource /*=nullptr*/)
	{
		return CreateTextureInternal(m_pTexture, pSource, false);
	}

	bool IOETexture_Platform::CreateStagingTexture(IOETextureSource *pSource /*=nullptr*/)
	{
		return CreateTextureInternal(m_pStagingTexture, pSource, true);
	}

	bool IOETexture_Platform::CreateTextureInternal(TComSharedPtr<ID3D11Resource> &rpResource,
													IOETextureSource *pSource /*=nullptr*/, bool bStaging /*=false*/)
	{
		ID3D11Device *pDevice(IOE::Renderer::IOERPI::GetSingletonPtr()->GetPlatform().GetDevice());
		rpResource.reset();
		if (pDevice == nullptr)
		{
			return false;
		...
		if (pSource != nullptr)
		{
			...
			pSubResouce = std::make_unique<D3D11_SUBRESOURCE_DATA[]>(uNumMipMaps);
			...
			for (uint32_t uIdx(0); uIdx < uNumMipMaps; ++uIdx)
			{
				...
				case ETextureType::Texture3D:
					rSubResource.SysMemSlicePitch = static_cast<UINT>(
						GetTexture().GetWidth() * GetTexture().GetHeight() * pSource->uPixelSizeBytes);
				// intentional fall through
				case ETextureType::Texture2D:

					rSubResource.SysMemPitch = static_cast<UINT>(GetTexture().GetWidth() * pSource->uPixelSizeBytes);
					break;
				}
			}
		}
		...
		if (GetTexture().IsDefaultRenderTarget())
		{
			...
			IDXGISwapChain *pSwapChain(IOE::Renderer::IOERPI::GetSingletonPtr()->GetPlatform().GetSwapChain());
			if (pSwapChain == nullptr)
			{
				return false;
			}
			if (FAILED(pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&rpResource)))
			{
				return false;
			}
			return true;
			...
			GetTexture().GetTextureType() == ETextureType::Texture1DArray)
		{
			...
			{
				if (FAILED(pDevice->CreateTexture1D(&tDesc, pSubResouce.get(), (ID3D11Texture1D**)&rpResource)))
				{
					return false;
				}
				return true;
				 ...
				 GetTexture().GetTextureType() == ETextureType::Texture2DArray)
		{
			...
			{
				if (FAILED(pDevice->CreateTexture2D(&tDesc, pSubResouce.get(), (ID3D11Texture2D**)&rpResource)))
				{
					return false;
				}
				return true;
		...
		else if (GetTexture().GetTextureType() == ETextureType::Texture3D)
		{
			...
			{
				if (FAILED(pDevice->CreateTexture3D(&tDesc, pSubResouce.get(), (ID3D11Texture3D**)&rpResource)))
				{
					return false;
				}
				return true;
				 ...
				 GetTexture().GetTextureType() == ETextureType::TextureCubeArray)
		{
			D3D11_TEXTURE2D_DESC tDesc;
		...
		else if (GetTexture().GetTextureType() == ETextureType::GenericBuffer)
		{
			...
			{
				if (FAILED(pDevice->CreateBuffer(&tDesc, pSubResouce.get(), (ID3D11Buffer**)&rpResource)))
				{
					return false;
				}
				return true;
	...
	bool IOETexture_Platform::CreateView()
	{
		...
		if (GetTexture().GetBufferDescription() & EBufferDescription::RenderTarget)
		{
				...
				GetTexture().GetTextureType() == ETextureType::TextureCubeArray)
			{
				...
				tDesc.Texture2DArray.MipSlice		 = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture2DArray.FirstArraySlice = GetTexture().GetFirstArrayIndexToMap() * 6 +
					(GetTexture().GetCubeFace() == ECubeFace::All ? 0 : static_cast<UINT>(GetTexture().GetCubeFace()));
				tDesc.Texture2DArray.ArraySize = GetTexture().GetCubeFace() == ECubeFace::All ? 6 : 1;
			}
			else
			{
				if (GetTexture().IsTextureArray())
				{
					tDesc.ViewDimension					 = D3D11_RTV_DIMENSION_TEXTURE2DARRAY;
					tDesc.Texture2DArray.MipSlice		 = static_cast<UINT>(GetTexture().GetMipToMap());
					tDesc.Texture2DArray.FirstArraySlice = static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap());
					tDesc.Texture2DArray.ArraySize		 = static_cast<UINT>(GetTexture().GetArrayCountToMap());
				}
				else
				{
					tDesc.ViewDimension		 = D3D11_RTV_DIMENSION_TEXTURE2D;
					tDesc.Texture2D.MipSlice = static_cast<UINT>(GetTexture().GetMipToMap());
				}
			}
			if (FAILED(pDevice->CreateRenderTargetView(m_pTexture.get(), &tDesc, (ID3D11RenderTargetView**)&m_pView)))
			{
				return false;
			}
			return true;
		}
		else if (GetTexture().GetBufferDescription() & EBufferDescription::DepthStencil)
		{
			...
			if (FAILED(pDevice->CreateDepthStencilView(m_pTexture.get(), &tDesc, (ID3D11DepthStencilView**)&m_pView)))
			{
				return false;
			}
			return true;
		}
		else if (GetTexture().GetBufferDescription() & EBufferDescription::UnorderedAccess)
		{
			...
			case ETextureType::Texture1D:
				...
				tDesc.Texture1D.MipSlice = static_cast<UINT>(GetTexture().GetMipToMap());
				break;
			case ETextureType::Texture1DArray:
				tDesc.ViewDimension					 = D3D11_UAV_DIMENSION_TEXTURE1DARRAY;
				tDesc.Texture1DArray.MipSlice		 = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture1DArray.FirstArraySlice = static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap());
				tDesc.Texture1DArray.ArraySize		 = static_cast<UINT>(GetTexture().GetArrayCountToMap());
				break;
			case ETextureType::Texture2D:
			...
			case ETextureType::Texture3D:
				...
				tDesc.Texture3D.MipSlice = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture3D.WSize	= -1;
				break;
			case ETextureType::TextureCubeArray:
				tDesc.ViewDimension					 = D3D11_UAV_DIMENSION_TEXTURE2DARRAY;
				tDesc.Texture2DArray.MipSlice		 = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture2DArray.FirstArraySlice = static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap()) * 6;
				tDesc.Texture2DArray.ArraySize		 = static_cast<UINT>(GetTexture().GetArrayCountToMap()) * 6;
				break;
			case ETextureType::GenericBuffer:
				tDesc.ViewDimension		  = D3D11_UAV_DIMENSION_BUFFER;
				tDesc.Buffer.FirstElement = static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap());
				tDesc.Buffer.NumElements  = static_cast<UINT>(GetTexture().GetArrayCountToMap());
				tDesc.Buffer.Flags		  = GetTexture().HasImplicitCounter() ? D3D11_BUFFER_UAV_FLAG_COUNTER : 0;
				if (GetTexture().CanAllowRawValues())
				{
			...
			if (FAILED(pDevice->CreateUnorderedAccessView(m_pTexture.get(), &tDesc,(ID3D11UnorderedAccessView**) &m_pView)))
			{
				return false;
			}
			return true;
		}
		else if (GetTexture().GetBufferDescription() == EBufferDescription::ShaderResource)
		{
			...
			case ETextureType::Texture1D:
				...
				tDesc.Texture1D.MostDetailedMip = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture1D.MipLevels		= IOE::Maths::Max(static_cast<UINT>(GetTexture().GetNumMips()), 1U);
				break;
			case ETextureType::Texture1DArray:
				tDesc.ViewDimension					 = D3D11_SRV_DIMENSION_TEXTURE1DARRAY;
				tDesc.Texture1DArray.MostDetailedMip = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture1DArray.MipLevels = IOE::Maths::Max(static_cast<UINT>(GetTexture().GetNumMips()), 1U);
				tDesc.Texture1DArray.FirstArraySlice = static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap());
				tDesc.Texture1DArray.ArraySize		 = static_cast<UINT>(GetTexture().GetArrayCountToMap());
				break;
			case ETextureType::Texture2D:
				tDesc.ViewDimension				= D3D11_SRV_DIMENSION_TEXTURE2D;
				tDesc.Texture2D.MostDetailedMip = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture2D.MipLevels		= IOE::Maths::Max(static_cast<UINT>(GetTexture().GetNumMips()), 1U);
				break;
			case ETextureType::Texture2DArray:
				tDesc.ViewDimension					 = D3D11_SRV_DIMENSION_TEXTURE2DARRAY;
				tDesc.Texture2DArray.MostDetailedMip = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture2DArray.MipLevels = IOE::Maths::Max(static_cast<UINT>(GetTexture().GetNumMips()), 1U);
				tDesc.Texture2DArray.FirstArraySlice = static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap());
				tDesc.Texture2DArray.ArraySize		 = static_cast<UINT>(GetTexture().GetArrayCountToMap());
				break;
			case ETextureType::Texture3D:
				tDesc.ViewDimension				= D3D11_SRV_DIMENSION_TEXTURE3D;
				tDesc.Texture3D.MostDetailedMip = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.Texture3D.MipLevels		= IOE::Maths::Max(static_cast<UINT>(GetTexture().GetNumMips()), 1U);
				break;
			case ETextureType::TextureCube:
				tDesc.ViewDimension				  = D3D_SRV_DIMENSION_TEXTURECUBE;
				tDesc.TextureCube.MostDetailedMip = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.TextureCube.MipLevels		  = IOE::Maths::Max(static_cast<UINT>(GetTexture().GetNumMips()), 1U);
				break;
			case ETextureType::TextureCubeArray:
				tDesc.ViewDimension					   = D3D_SRV_DIMENSION_TEXTURECUBEARRAY;
				tDesc.TextureCubeArray.MostDetailedMip = static_cast<UINT>(GetTexture().GetMipToMap());
				tDesc.TextureCubeArray.MipLevels = IOE::Maths::Max(static_cast<UINT>(GetTexture().GetNumMips()), 1U);
				tDesc.TextureCubeArray.First2DArrayFace =
					static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap()) * 6;
				tDesc.TextureCubeArray.NumCubes = static_cast<UINT>(GetTexture().GetArrayCountToMap());
				break;
			case ETextureType::GenericBuffer:
				if (GetTexture().CanAllowRawValues())
				{
					tDesc.ViewDimension			= D3D_SRV_DIMENSION_BUFFEREX;
					tDesc.BufferEx.FirstElement = static_cast<UINT>(GetTexture().GetFirstArrayIndexToMap());
					tDesc.BufferEx.NumElements  = static_cast<UINT>(GetTexture().GetArrayCountToMap());
			...
			if (FAILED(pDevice->CreateShaderResourceView(m_pTexture.get(), &tDesc, (ID3D11ShaderResourceView**)&m_pView)))
			{
				return false;
			}
			return true;
	...
	void IOETexture_Platform::LoadFromMemory(std::uint8_t *pMemoryAddress, std::uint32_t uWidth, std::uint32_t uHeight,
											 std::uint32_t uPixelByteWidth /*=4*/)
	{
	}
	...
	{
		IOE_ASSERT(rhs.GetPlatform().m_pTexture != nullptr, "Texture not yet set up!");
	...
	*pFmt, ...)
	...
	void IOETexture_Platform::SetDebugName(const std::string &szName)
	{
		...
		if (m_pTexture != nullptr)
		{
			SetD3DObjectDebugName(m_pTexture.get(), "%s Texture", szName.c_str());
		}
	}
