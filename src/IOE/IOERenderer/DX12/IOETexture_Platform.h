...
namespace Renderer
{
	...
	public:
		ID3D12Resource *GetD3D11Texture()
		{
			return m_pTexture.get();
		}

		const ID3D12Resource *GetD3D11Texture() const
		{
			return m_pTexture.get();
		}

		TComSharedPtr<ID3D12Resource> GetD3D11TextureShared()
		{
			return m_pTexture;
		}

		const TComSharedPtr<ID3D12Resource> GetD3D11TextureShared() const
		{
			return m_pTexture;
		}
