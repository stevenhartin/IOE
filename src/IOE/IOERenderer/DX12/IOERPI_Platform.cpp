...
namespace Renderer
{
	...
	void IOERPI_Platform::FinishRender()
	{
		...
		if (bRenderDebug)
		{
			...
			m_pD3DDeviceContext->IASetVertexBuffers(0, 1, &m_pVertexBuffer, &uStride, &uRendered);
	...
	void IOERPI_Platform::CreateDebugBuffers()
	{
		...
		if (!SUCCEEDED(m_pD3DDevice->CreateBuffer(&tBufferDesc, nullptr, &pBuffer)))
		{
	...
	int32_t IOERPI_Platform::GetFirstUAVSlotIndex() const
	{
		if (m_nNumUAVs <= 0)
		{
			return 0;
		}
		int32_t nFirstSlot(m_arrUAVs[0].second);
		for (int32_t nIdx(1); nIdx < m_nNumUAVs; ++nIdx)
		{
			if (m_arrUAVs[nIdx].first != nullptr)
			{
				if (m_arrUAVs[nIdx].second < nFirstSlot)
				{
					nFirstSlot = m_arrUAVs[nIdx].second;
				}
			}
		}
		return nFirstSlot;
	...
	int32_t IOERPI_Platform::GetMaxUAVIndex() const
	{
		...
		int32_t nMaxSlot(m_arrUAVs[0].second);
		for (int32_t nIdx(1); nIdx < m_nNumUAVs; ++nIdx)
		{
			if (m_arrUAVs[nIdx].first != nullptr)
			{
				if (m_arrUAVs[nIdx].second > nMaxSlot)
				{
					nMaxSlot = m_arrUAVs[nIdx].second;
				}
			}
		}
		return nMaxSlot;
	...
	void IOERPI_Platform::RefreshRenderTargetsAndUAVs()
	{
		...
		else
		{
			...
			/*m_pD3DDeviceContext->OMSetRenderTargetsAndUnorderedAccessViews(
				...
				GetNumRenderTargets(), GetNumUAVs(), arrUAVs.data(),
				m_arrUAVsWriteOffsets);*/
			m_pD3DDeviceContext->OMSetRenderTargets(GetNumRenderTargets(),
													arrRenderTargets.data(),
													m_pD3DDepthStencilView.get());
		}
#endif
	}
