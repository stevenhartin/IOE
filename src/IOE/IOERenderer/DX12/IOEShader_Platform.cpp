...
namespace Renderer
{
	...
	void
	IOEShader_Platform::Compile(const IOEShaderCompilationOptions &rOptions)
	{
		std::cout << "Compiling shader: "
				  << IOE::Core::Algorithm::ConvertWideToNarrow(rOptions.szPath)
						 .c_str()
		...
		{
			std::copy(std::begin(*rOptions.pMacros),
					  std::end(*rOptions.pMacros),
					  std::back_inserter(m_arrMacros));
		}
		...
		if (nNumMacros > 0)
		{
			...
			for (std::size_t nIdx(0); nIdx < nNumMacros; ++nIdx)
			{
				...
				arrMacros[nIdx].Definition =
					GetMacros().at(nIdx).m_szDefinition.c_str();
			}
		...
		if (FAILED(tResult))
		{
			...
			strncpy_s<uBufferSize>(
				buffer, pErrors != nullptr
			...
			THROW_IOE_RENDERER_EXCEPTION_VARGS(
				...
				E_ERROR_FAILED_TO_COMPILE_SHADER, buffer);
		}
		...
		if (FAILED(tResult))
		{
			THROW_IOE_RENDERER_EXCEPTION("Failed to create shader",
	...
	void IOEShader_Platform::CreateInputLayout()
	{
		...
		IOEVertexFormat *pVertexFormat(
			m_rShader.GetVertexFormat());

		if (FAILED(rPlatform.GetDevice()->CreateInputLayout(
				pVertexFormat->GetPlatform().GetInputDescription().data(),
				pVertexFormat->GetNumSemantics(),
				m_pCompiledShader->GetBufferPointer(),
				m_pCompiledShader->GetBufferSize(), &m_pInputLayout)))
		{
	...
	void IOEShader_Platform::FindShaderVariables(
		...
		ShaderCategories &arrDescriptions)
	{
		...
		{
			THROW_IOE_RENDERER_EXCEPTION(
				"Failed to get shader reflectioin parameters",
		...
		for (std::uint32_t uBufferIdx(0);
			 uBufferIdx < tShaderDesc.BoundResources; ++uBufferIdx)
		{
			D3D11_SHADER_INPUT_BIND_DESC bufferBindDesc;
			if (FAILED(pShaderVars->GetResourceBindingDesc(uBufferIdx,
														   &bufferBindDesc)))
			{
				THROW_IOE_RENDERER_EXCEPTION(
					"Failed to get description for a resource",
					E_ERROR_FAILED_TO_FIND_VARIABLES);
			}
			if (bufferBindDesc.Type == D3D_SIT_CBUFFER ||
				bufferBindDesc.Type == D3D_SIT_TBUFFER)
			{
				// Handled below
				continue;
			}
		...
		for (std::uint32_t uBufferIdx(0);
			 uBufferIdx < tShaderDesc.ConstantBuffers; ++uBufferIdx)
		{
			ID3D11ShaderReflectionConstantBuffer *pConstantBuffer =
				pShaderVars->GetConstantBufferByIndex(uBufferIdx);

			D3D11_SHADER_BUFFER_DESC bufferDesc;
			if (FAILED(pConstantBuffer->GetDesc(&bufferDesc)))
			{
				THROW_IOE_RENDERER_EXCEPTION(
					"Failed to get description for a constant buffer",
					E_ERROR_FAILED_TO_FIND_VARIABLES);
			}

			D3D11_SHADER_INPUT_BIND_DESC bufferBindDesc;
			if (FAILED(pShaderVars->GetResourceBindingDescByName(
					bufferDesc.Name, &bufferBindDesc)))
			{
				THROW_IOE_RENDERER_EXCEPTION_VARGS(
					"Failed to get resource binding for constant buffer '%s'",
					E_ERROR_FAILED_TO_FIND_VARIABLES, bufferDesc.Name);
			}

			if (bufferBindDesc.BindPoint >=
			...
			{
				ShaderConstantBufferDescription *pCBuffer =
					&arrConstantBuffers[bufferBindDesc.BindPoint];
				...
				tBufferDesc.ByteWidth =
					static_cast<UINT>(pCBuffer->nSizeBytes);
				...
				if (FAILED(IOERPI::GetSingletonPtr()
							   ->GetPlatform()
							   .GetDevice()
							   ->CreateBuffer(&tBufferDesc, nullptr,
											  &m_arrConstantD3D11Buffers
												  [bufferBindDesc.BindPoint])))
				{
					THROW_IOE_RENDERER_EXCEPTION_VARGS(
						"Failed to create D3D11 Buffer to map CBuffer '%s'",
						E_ERROR_FAILED_TO_FIND_VARIABLES, pCBuffer->szName);
				}

				for (std::uint32_t uVariableIdx(0);
					 uVariableIdx < bufferDesc.Variables; ++uVariableIdx)
				{
					...
					if (!pShaderVariable)
					{
						THROW_IOE_RENDERER_EXCEPTION_VARGS(
							...
							"constant buffer %s",
							E_ERROR_FAILED_TO_FIND_VARIABLES, uVariableIdx,
							pCBuffer->szName);
					}

					D3D11_SHADER_VARIABLE_DESC tVariableDescription;
					if (FAILED(
							pShaderVariable->GetDesc(&tVariableDescription)))
					{
						THROW_IOE_RENDERER_EXCEPTION_VARGS(
					...
					ID3D11ShaderReflectionType *pShaderVariableType(
						pShaderVariable->GetType());
					...
					if (FAILED(pShaderVariableType->GetDesc(
							&tShaderVariableTypeDesc)))
					{
						THROW_IOE_RENDERER_EXCEPTION_VARGS(
					...
					EShaderVariableType eShaderVariableType(
						EShaderVariableType::None);

					if (tShaderVariableTypeDesc.Type == D3D_SVT_FLOAT &&
						tShaderVariableTypeDesc.Rows == 1 &&
						tShaderVariableTypeDesc.Columns == 1)
					{
						eShaderVariableType = EShaderVariableType::Float;
					}
					else if (tShaderVariableTypeDesc.Type == D3D_SVT_FLOAT &&
							 tShaderVariableTypeDesc.Rows == 1 &&
							 ...
							 tShaderVariableTypeDesc.Columns == 4)
					{
						eShaderVariableType = EShaderVariableType::Float4;
							 ...
							 tShaderVariableTypeDesc.Rows == 4 &&
							 tShaderVariableTypeDesc.Columns == 4)
					{
						eShaderVariableType = EShaderVariableType::Float4x4;
					}
					else if (tShaderVariableTypeDesc.Type == D3D_SVT_INT &&
							 tShaderVariableTypeDesc.Rows == 1 &&
							 tShaderVariableTypeDesc.Columns == 1)
					{
						eShaderVariableType = EShaderVariableType::Int;
					}
					else if (tShaderVariableTypeDesc.Type == D3D_SVT_UINT &&
							 tShaderVariableTypeDesc.Rows == 1 &&
							 tShaderVariableTypeDesc.Columns == 1)
					{
					...
					if (eShaderVariableType != EShaderVariableType::None)
					{
						...
						strcpy_s<64>(tDescription.szName,
									 _strdup(tVariableDescription.Name));
	...
	void IOEShader_Platform::FindGlobalResource(
		ID3D11ShaderReflection *pShaderReflection, int nInputType,
		...
		ShaderCategories &arrGlobalDescriptions)
	{
		...
		{
			THROW_IOE_RENDERER_EXCEPTION(
				"Failed to find shader reflection description",
				E_ERROR_FAILED_TO_FIND_VARIABLES);
		}

		for (std::uint32_t uResourceIdx(0);
			 uResourceIdx < tShaderDesc.BoundResources; ++uResourceIdx)
		{
			D3D11_SHADER_INPUT_BIND_DESC tInputBindDesc;
			if (FAILED(pShaderReflection->GetResourceBindingDesc(
					uResourceIdx, &tInputBindDesc)))
			{
				THROW_IOE_RENDERER_EXCEPTION_VARGS(
			...
			if (tInputBindDesc.Type == nInputType)
			{
					...
					eVariableType == EShaderVariableType::TextureCubeArray)
				{
					...
					case D3D_SRV_DIMENSION_TEXTURE1DARRAY:
						tDescription.eType =
							EShaderVariableType::Texture1DArray;
						break;
					case D3D_SRV_DIMENSION_TEXTURE2DARRAY:
						tDescription.eType =
							EShaderVariableType::Texture2DArray;
						break;
					case D3D_SRV_DIMENSION_TEXTURECUBEARRAY:
						tDescription.eType =
							EShaderVariableType::TextureCubeArray;
						break;
					}
				}
				else
				...
				arrGlobalDescriptions[EShaderVariableCategories::Texture]
					.push_back(tDescription);
			}
		}
	}
	...
	void IOEShader_Platform::InsertTexture(
		const D3D11_SHADER_INPUT_BIND_DESC &rDesc,
		ShaderCategories &arrGlobalDescriptions)
	{
	...
	void IOEShader_Platform::InsertSampler(
		const D3D11_SHADER_INPUT_BIND_DESC &rDesc,
		ShaderCategories &arrGlobalDescriptions)
	{
		if (rDesc.Type == D3D_SIT_SAMPLER)
		{
			...
			arrGlobalDescriptions[EShaderVariableCategories::Sampler]
				.push_back(tDescription);
		}
	}

	void IOEShader_Platform::InsertRawBuffer(
		const D3D11_SHADER_INPUT_BIND_DESC &rDesc,
		ShaderCategories &arrGlobalDescriptions)
	{
		if (rDesc.Type == D3D_SIT_BYTEADDRESS ||
			rDesc.Type == D3D_SIT_UAV_RWBYTEADDRESS)
		{
			...
			tDescription.nSlot			= rDesc.BindPoint;
			tDescription.nSizeOrSamples = rDesc.NumSamples;
			tDescription.bGPUWriteable =
				rDesc.Type == D3D_SIT_UAV_RWBYTEADDRESS;

			arrGlobalDescriptions[EShaderVariableCategories::RawBuffer]
				.push_back(tDescription);
		}
	}

	void IOEShader_Platform::InsertStructuredBuffer(
		const D3D11_SHADER_INPUT_BIND_DESC &rDesc,
		ShaderCategories &arrGlobalDescriptions)
	{
		if (rDesc.Type == D3D_SIT_STRUCTURED ||
			...
			rDesc.Type == D3D_SIT_UAV_RWSTRUCTURED_WITH_COUNTER)
		{
			...
			arrGlobalDescriptions[EShaderVariableCategories::StructuredBuffer]
				.push_back(tDescription);
		}
	}
	...
	void *IOEShader_Platform::MapConstantBuffers(
		...
		ShaderConstantBufferDescription *pBuffer)
	{
		if (m_arrConstantD3D11Buffers[pBuffer->nSlotIdx] != nullptr)
		{
			...
			if (SUCCEEDED(pRPI->GetPlatform().GetDeviceContext()->Map(
					m_arrConstantD3D11Buffers[pBuffer->nSlotIdx].get(), 0,
					D3D11_MAP_WRITE_DISCARD, 0, &tBufferMap)))
			{
				return static_cast<void *>(
					static_cast<std::uint8_t *>(tBufferMap.pData) + nSlotIdx);
			}
		}
		return nullptr;
	}
	...
	void IOEShader_Platform::UnmapConstantBuffers(
		IOERPI *pRPI, std::int32_t nSlotIdx,
		ShaderConstantBufferDescription *pBuffer)
	{
		if (m_arrConstantD3D11Buffers[pBuffer->nSlotIdx] != nullptr)
		{
			pRPI->GetPlatform().GetDeviceContext()->Unmap(
				m_arrConstantD3D11Buffers[pBuffer->nSlotIdx].get(), 0);
		}
	}
	...
	void IOEShader_Platform::Set(IOERPI *pRPI)
	{
		std::array<ID3D11Buffer*,
			ShaderConstantBufferDescription::ms_nMaxCBufferSlots>
			arrConstantD3D11Buffers;
		std::transform(
			std::begin(m_arrConstantD3D11Buffers),
			std::end(m_arrConstantD3D11Buffers),
		...
		{
		case EShaderType::Vertex:
			pRPI->GetPlatform().GetDeviceContext()->VSSetConstantBuffers(
				0, m_uNumCBuffers, pBuffer);
			pRPI->GetPlatform().GetDeviceContext()->VSSetShader(
				reinterpret_cast<ID3D11VertexShader *>(m_pCreatedShader.get()),
				nullptr, 0);
			pRPI->GetPlatform().GetDeviceContext()->IASetInputLayout(
				m_pInputLayout.get());
			break;
		case EShaderType::Geometry:
			pRPI->GetPlatform().GetDeviceContext()->GSSetConstantBuffers(
				0, m_uNumCBuffers, pBuffer);
			pRPI->GetPlatform().GetDeviceContext()->GSSetShader(
				reinterpret_cast<ID3D11GeometryShader *>(m_pCreatedShader.get()),
				nullptr, 0);
			break;
		case EShaderType::Hull:
			pRPI->GetPlatform().GetDeviceContext()->HSSetConstantBuffers(
				0, m_uNumCBuffers, pBuffer);
			pRPI->GetPlatform().GetDeviceContext()->HSSetShader(
				reinterpret_cast<ID3D11HullShader *>(m_pCreatedShader.get()),
				nullptr, 0);
			break;
		case EShaderType::Domain:
			pRPI->GetPlatform().GetDeviceContext()->DSSetConstantBuffers(
				0, m_uNumCBuffers, pBuffer);
			pRPI->GetPlatform().GetDeviceContext()->DSSetShader(
				reinterpret_cast<ID3D11DomainShader *>(m_pCreatedShader.get()),
				nullptr, 0);
			break;
		case EShaderType::Fragment:
			pRPI->GetPlatform().GetDeviceContext()->PSSetConstantBuffers(
				0, m_uNumCBuffers, pBuffer);
			pRPI->GetPlatform().GetDeviceContext()->PSSetShader(
				reinterpret_cast<ID3D11PixelShader *>(m_pCreatedShader.get()),
				nullptr, 0);
			break;
		case EShaderType::Compute:
			pRPI->GetPlatform().GetDeviceContext()->CSSetShader(
				reinterpret_cast<ID3D11ComputeShader *>(m_pCreatedShader.get()),
				nullptr, 0);
			pRPI->GetPlatform().GetDeviceContext()->CSSetConstantBuffers(
				0, m_uNumCBuffers, pBuffer);
			break;
		}
	}
	...
	bool IOEShader_Platform::SetTextureVariable(IOERPI *pRPI,
												...
												int arrOffset /*=0*/)
	{
		...
		{
			bGenericTexture =
				(pTexture->GetTextureType() == ETextureType::GenericBuffer);
		}
		else
		{

			bGenericTexture =
				m_rShader.FindGlobalVariable(
					szTextureName, EShaderVariableCategories::RawBuffer) !=
					nullptr ||
				m_rShader.FindGlobalVariable(
					szTextureName,
					EShaderVariableCategories::StructuredBuffer) != nullptr;
		}
		if (bGenericTexture)
		{
		...
		{
			return SetShaderResourceVariable(pRPI, szTextureName, pTexture,
											 arrOffset);
		}
		return false;
	}
	...
	bool IOEShader_Platform::SetShaderResourceVariable(
		IOERPI *pRPI, const char *szTextureName, IOETexture *pTexture,
		int arrOffset /*=0*/)
	{
		ShaderVariableDescription *pDescription(m_rShader.FindGlobalVariable(
			szTextureName, EShaderVariableCategories::Texture));
		if (pDescription == nullptr)
		{
			return false;
		}
		IOE_ASSERT(!pDescription->bGPUWriteable,
				   "Writeable textures not yet supported!");
		ID3D11ShaderResourceView *pShaderResourceView(
			pTexture ? static_cast<ID3D11ShaderResourceView *>(
						   pTexture->GetPlatform().GetView())
					 : nullptr);
		switch (m_rShader.GetShaderType())
		{
		case EShaderType::Vertex:
			pRPI->GetPlatform().GetDeviceContext()->VSSetShaderResources(
				pDescription->nSlot, 1, &pShaderResourceView);
			return true;
		case EShaderType::Geometry:
			pRPI->GetPlatform().GetDeviceContext()->GSSetShaderResources(
				pDescription->nSlot, 1, &pShaderResourceView);
			return true;
		case EShaderType::Hull:
			pRPI->GetPlatform().GetDeviceContext()->HSSetShaderResources(
				pDescription->nSlot, 1, &pShaderResourceView);
			return true;
		case EShaderType::Domain:
			pRPI->GetPlatform().GetDeviceContext()->DSSetShaderResources(
				pDescription->nSlot, 1, &pShaderResourceView);
			return true;
		case EShaderType::Fragment:
			pRPI->GetPlatform().GetDeviceContext()->PSSetShaderResources(
				pDescription->nSlot, 1, &pShaderResourceView);
			return true;
		case EShaderType::Compute:
			pRPI->GetPlatform().GetDeviceContext()->CSSetShaderResources(
				pDescription->nSlot, 1, &pShaderResourceView);
			return true;
		}
		return false;
	...
	bool IOEShader_Platform::SetBufferVariable(IOERPI *pRPI,
											   const char *szTextureName,
											   IOETexture *pTexture,
											   int arrOffset /*=0*/)
	{
		...
		if (pDescription == nullptr)
		{
			pDescription = m_rShader.FindGlobalVariable(
				szTextureName, EShaderVariableCategories::StructuredBuffer);
			if (pDescription == nullptr)
			{
				return false;
			}
		...
		{
			EUAVSet eUAVSet(m_rShader.GetShaderType() == EShaderType::Compute
								? EUAVSet::ComputeShader
								: EUAVSet::OutputMerger);
			if (pTexture)
			{
				IOE_ASSERT(pTexture->GetBufferDescription() ==
							   EBufferDescription::UnorderedAccess,
						   "Writeable texture must be bound as a UAV");
				pRPI->SetUAV(*pTexture, pDescription->nSlot, eUAVSet,
							 arrOffset);
				return true;
			}
			else
		...
		{
			IOE_ASSERT(pTexture == nullptr ||
						   pTexture->GetBufferDescription() ==
	...
	bool IOEShader_Platform::SetSamplerVariable(IOERPI *pRPI,
												const char *szSamplerName,
												IOESamplerState *pSamplers)
	{
		ShaderVariableDescription *pDescription(m_rShader.FindGlobalVariable(
			szSamplerName, EShaderVariableCategories::Sampler));
		if (pDescription == nullptr)
		{
			return false;
		}
		ID3D11SamplerState *pSamplerState(
			pSamplers ? pSamplers->GetPlatform().GetD3D11State() : nullptr);
		switch (m_rShader.GetShaderType())
		{
		case EShaderType::Vertex:
			pRPI->GetPlatform().GetDeviceContext()->VSSetSamplers(
				pDescription->nSlot, 1, &pSamplerState);
			return true;
		case EShaderType::Geometry:
			pRPI->GetPlatform().GetDeviceContext()->GSSetSamplers(
				pDescription->nSlot, 1, &pSamplerState);
			return true;
		case EShaderType::Hull:
			pRPI->GetPlatform().GetDeviceContext()->HSSetSamplers(
				pDescription->nSlot, 1, &pSamplerState);
			return true;
		case EShaderType::Domain:
			pRPI->GetPlatform().GetDeviceContext()->DSSetSamplers(
				pDescription->nSlot, 1, &pSamplerState);
			return true;
		case EShaderType::Fragment:
			pRPI->GetPlatform().GetDeviceContext()->PSSetSamplers(
				pDescription->nSlot, 1, &pSamplerState);
			return true;
		case EShaderType::Compute:
			pRPI->GetPlatform().GetDeviceContext()->CSSetSamplers(
				pDescription->nSlot, 1, &pSamplerState);
			return true;
		}
		return false;
	...
	bool IOEShader_Platform::SetSamplers(
		IOERPI *pRPI,
		const vector<pair<string, IOESamplerState *> > &arrSamplers)
	{
		...
		for (const auto &tPair : arrSamplers)
		{
			ShaderVariableDescription *pDescription(
				m_rShader.FindGlobalVariable(
					tPair.first.c_str(), EShaderVariableCategories::Sampler));
			if (pDescription == nullptr)
			{
				return false;
			}
			ID3D11SamplerState *pSamplerState(
				tPair.second ? tPair.second->GetPlatform().GetD3D11State()
							 : nullptr);
		...
		if (bAnySet && uNumSamplers > 0)
		{
			...
			case EShaderType::Vertex:
				pRPI->GetPlatform().GetDeviceContext()->VSSetSamplers(
					0, uNumSamplers, arrD3D11Samplers);
				bAnySet = true;
				break;
			case EShaderType::Geometry:
				pRPI->GetPlatform().GetDeviceContext()->GSSetSamplers(
	...
	void IOEShader_Platform::Dispatch(IOERPI *pRPI, int32_t nNumThreadGroupsX,
									  int32_t nNumThreadGroupsY,
									  int32_t nNumThreadGroupsZ)
	{
		pRPI->GetPlatform().GetDeviceContext()->Dispatch(
			(UINT)nNumThreadGroupsX, (UINT)nNumThreadGroupsY,
