...
template<typename Props> bool LargeObjectCacheImpl<Props>::CacheBin::CacheBinFunctor::
    OperationPreprocessor::getFromPutList(CacheBinOperation *opGet_, uintptr_t currTime_)
{
    if ( head ) {
        uintptr_t age = head->age;
        LargeMemoryBlock *next = head->next;
        *opCast<OpGet>(*opGet_).res = head;
        commitOperation( opGet_ );
        head = next;
        putListNum--;
        MALLOC_ASSERT( putListNum>=0, ASSERT_TEXT );

        // use moving average with current hit interval
        bin->updateMeanHitRange( currTime_ - age );
        return true;
    }
    return false;
}
    ...
    OperationPreprocessor::operator()(CacheBinOperation* opList)
{
    for ( CacheBinOperation *op = opList, *opNext; op; op = opNext ) {
        ...
        case CBOP_PUT_LIST:
            {
                LargeMemoryBlock *head_ = opCast<OpPutList>(*op).head;
                LargeMemoryBlock *curr = head_, *prev = NULL;
                ...
                } while (( curr = curr->next ) != false);

                LargeMemoryBlock *tail_ = prev;
                addToPutList(head_, tail_, num);
        ...
        case CBOP_CLEAN_TO_THRESHOLD:
            {
                uintptr_t currTime_ = opCast<OpCleanToThreshold>(*op).currTime;
                // We don't worry about currTime overflow since it is a rare
                // occurrence and doesn't affect correctness
                cleanTime = cleanTime < currTime_ ? currTime_ : cleanTime;
                addOpToOpList( op, &opClean );
            }
            break;
...
template<typename Props> void LargeObjectCacheImpl<Props>::
    CacheBin::ExecuteOperation(CacheBinOperation *op, ExtMemoryPool *extMemPool, BinBitMask *bitMask_, int idx, bool longLifeTime)
{
    CacheBinFunctor func( this, extMemPool, bitMask_, idx );
...
template<typename Props> LargeMemoryBlock *LargeObjectCacheImpl<Props>::
    CacheBin::get(ExtMemoryPool *extMemPool, size_t size, BinBitMask *bitMask_, int idx)
{
    LargeMemoryBlock *lmb=NULL;
    OpGet data = {&lmb, size};
    CacheBinOperation op(data);
    ExecuteOperation( &op, extMemPool, bitMask_, idx );
    return lmb;
}

template<typename Props> void LargeObjectCacheImpl<Props>::
    CacheBin::putList(ExtMemoryPool *extMemPool, LargeMemoryBlock *head, BinBitMask *bitMask_, int idx)
{
    ...
    ExecuteOperation( op, extMemPool, bitMask_, idx, false );
}

template<typename Props> bool LargeObjectCacheImpl<Props>::
    CacheBin::cleanToThreshold(ExtMemoryPool *extMemPool, BinBitMask *bitMask_, uintptr_t currTime, int idx)
{
    ...
    bool released = toRelease != nullptr;
...
template<typename Props> bool LargeObjectCacheImpl<Props>::
    CacheBin::cleanAll(ExtMemoryPool *extMemPool, BinBitMask *bitMask_, int idx)
{
    LargeMemoryBlock *toRelease = NULL;
...
template<typename Props> void LargeObjectCacheImpl<Props>::
    CacheBin::decrUsedSize(ExtMemoryPool *extMemPool, size_t size, BinBitMask *bitMask_, int idx) {
...
template<typename Props> LargeMemoryBlock *LargeObjectCacheImpl<Props>::
    CacheBin::putList(LargeMemoryBlock *head, LargeMemoryBlock *tail, BinBitMask *bitMask_, int idx, int num)
{
    ...
    CacheBin::forgetOutdatedState(uintptr_t currTime)
{
    ...
    if (ageThreshold)
        doCleanup = (intptr_t)sinceLastGet > Props::LongWaitFactor*ageThreshold;
...
template<typename Props> LargeMemoryBlock *LargeObjectCacheImpl<Props>::
    CacheBin::cleanToThreshold(uintptr_t currTime, BinBitMask *bitMask_, int idx)
{
    /* oldest may be more recent then age, that's why cast to signed type
    was used. age overflow is also processed correctly. */
...
template<typename Props> LargeMemoryBlock *LargeObjectCacheImpl<Props>::
    CacheBin::cleanAll(BinBitMask *bitMask_, int idx)
{
    if (!last) return NULL;
