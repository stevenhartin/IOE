...
protected:
    ...
    int getMinTrue(unsigned startIdx) const {
        ...
        for (int i=(int)idx; i<SZ; i++, curr=mask[i]) {
            if (-1 != (pos = (int)BitScanRev(curr)))
...
private:
  ...
  /* time of last get called for the bin */
        ...
        class CacheBinFunctor {
            ...
            public:
                ...
                uintptr_t getTimeRange() const { return (uintptr_t)(-(intptr_t)lclTime); }
  ...
  /* ---------- unsafe methods used with the agreggator ---------- */
        ...
        LargeMemoryBlock *putList(LargeMemoryBlock *head, LargeMemoryBlock *tail, BinBitMask *bitMask_, int idx, int num);
        LargeMemoryBlock *get();
        LargeMemoryBlock *cleanToThreshold(uintptr_t currTime, BinBitMask *bitMask, int idx);
        LargeMemoryBlock *cleanAll(BinBitMask *bitMask_, int idx);
        void updateUsedSize(size_t size, BinBitMask *bitMask_, int idx) {
            if (!usedSize) bitMask_->set(idx, true);
            usedSize += size;
            if (!usedSize && !first) bitMask_->set(idx, false);
        }
...
public:
    static int sizeToIdx(size_t size) {
        ...
        return (int)((size-Props::MinSize)/Props::CacheStep);
    }
...
private:
    static int sizeToBin(size_t size) {
        ...
        int bin = ((int)size - (int)minBinnedSize)/ (int)LargeObjectCache::largeBlockCacheStep;
...
struct ExtMemoryPool {
    ...
    bool userPool() const { return rawAlloc != nullptr; }
