...
void* getRawMemory (size_t size, bool hugePages_) {
    return MapMemory(size, hugePages_);
}

bool freeRawMemory (void *object, size_t size) {
    return UnmapMemory(object, size) != 0;
}
...
void HugePagesStatus::printStatus() {
    doPrintStatus(requestedMode.get() != 0, "requested");
...
void *Backend::getRawMem(size_t &size) const
{
    ...
    if (extMemPool->userPool()) {
        ...
        if (FencedLoad(hugePages.enabled)) {
            ...
            hugePages.registerAllocation(res != nullptr);
        }
...
{
    AtomicAdd((intptr_t&)totalMemSize, -(intptr_t)size);
    if (extMemPool->userPool())
...
bool Backend::scanCoalescQ(bool forceCoalescQDrop)
{
    ...
    if (currCoalescList != nullptr)
        coalescAndPutList(currCoalescList, forceCoalescQDrop);
    return currCoalescList != nullptr;
}
...
bool Backend::bootstrap(ExtMemoryPool *extMemoryPool)
{
    ...
    return addNewRegion(2*1024*1024, /*exact=*/false, /*addToBin=*/true) != nullptr;
}
