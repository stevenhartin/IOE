...
#define RESET_PTHREAD 0
#if !defined(USE_PTHREAD)
#undef RESET_PTHREAD
#define RESET_PTHREAD 1
#define USE_PTHREAD 0
    ...
    #define GetMyTID() GetCurrentThreadId()
		...
		#include<thread>
			#define TlsSetValue_func Fls0SetValue
 ...
 */
const uint16_t startupAllocObjSizeMark = (uint16_t)~(uint16_t)0U;
...
#else
	#if !defined(STAT_increment)
		#define STAT_increment(tid, bin, ctr)	\
			UNREFERENCED_PARAMETER(tid);		\
			UNREFERENCED_PARAMETER(bin);		\
			UNREFERENCED_PARAMETER(ctr);		
	#endif
#endif
...
public:
    ...
    bool freeListNonNull() { return freeList != nullptr; }
...
{
	static bool indexRequest_ = indexRequest;
...
template<bool poolDestroy> void AllLargeBlocksList::releaseAll(Backend *backend) {
	 ...
	 static bool poolDestroy_ = poolDestroy;
...
bool ExtMemoryPool::init(intptr_t poolId_, rawAllocType rawAlloc_,
                         rawFreeType rawFree_, size_t granularity_,
                         bool keepAllMemory_, bool fixedPool_)
{
    this->poolId = poolId_;
    this->rawAlloc = rawAlloc_;
    this->rawFree = rawFree_;
    this->granularity = granularity_;
    this->keepAllMemory = keepAllMemory_;
    this->fixedPool = fixedPool_;
...
void Block::initEmptyBlock(TLSData *tls, size_t size)
{
    ...
    unsigned int index = getIndex((unsigned int)size);
    unsigned int objSz = getObjectSize((unsigned int)size);

    cleanBlockHeader();
    objectSize = (uint16_t)(objSz & 0xFFFF);
    markOwned(tls);
...
LargeMemoryBlock *LocalLOCImpl<LOW_MARK, HIGH_MARK>::get(size_t size)
{
    ...
    if (!head)
		return NULL;
	localHead = (LargeMemoryBlock*)AtomicFetchStore(&head, 0);
	if (!localHead) {
    ...
    if (lmb) {
        ...
        if (ptrDelta && tls) { // !tls is cold path
            ...
            unsigned numOfPossibleOffsets = alignment == estimatedCacheLineSize?
                  (unsigned)(ptrDelta / estimatedCacheLineSize) :
...
FreeObject *Block::findAllocatedObject(const void *address) const
{
    ...
    uint16_t offset = (uint16_t)(((uintptr_t)this + slabSize - (uintptr_t)address) & 0xFFFF);
...
static void *internalPoolMalloc(MemoryPool* memPool, size_t size)
{
     ...
     */
    mallocBlock = memPool->orphanedBlocks.get(tls, (unsigned int)size);
    while (mallocBlock) {
