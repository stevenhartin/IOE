...
float4 main(PS_INPUT input) : SV_TARGET0
{
	...
	float3 V = normalize(vEye - input.WPos.xyz);
	float3 L = normalize(g_LightPos - input.WPos.xyz);
    ...
    const float fRoughness = 0.5f;
	const float fEnergy = 0.05f;
	...
	float diffuseIntensity = dot(normalize(mul(L, (float3x3)WorldInvTranspose)), bumpNrm);
    if(diffuseIntensity < 0)
	{
        diffuseIntensity = 0;
	}
